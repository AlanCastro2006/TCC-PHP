                                                    ...
                                                    <form action="/cards/{{ $card->id }}" method="POST" enctype="multipart/form-data">
                                                        ...
                                                        <div class="accordion" id="accordionEdit">
                                                            ...
                                                            <div class="accordion-item">
                                                                <h2 class="accordion-header">
                                                                    <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                                                        data-bs-target="#collapseOne{{ $card->id }}" aria-expanded="true"
                                                                        aria-controls="collapseOne{{ $card->id }}">
                                                                        Informações da Peça
                                                                    </button>
                                                                </h2>

                                                                <div id="collapseOne{{ $card->id }}" class="accordion-collapse collapse show"
                                                                    data-bs-parent="#accordionEdit">
                                                                    <div class="accordion-body">

                                                                        <div class="form-floating mb-3">
                                                                            <input type="text" class="form-control" required name="name"
                                                                                id="name{{ $card->id }}" placeholder="Nome" value="{{ $card->name }}">
                                                                            <label for="floatingInput">Nome</label>
                                                                        </div>
                                                                        <div class="form-floating mb-3">
                                                                            <input type="url" class="form-control" required
                                                                        ...
                                                                        <div class="form-floating mb-3">
                                                                            <textarea class="form-control" required name="description" id="description" placeholder="Descrição">{{ $card->description }}</textarea>
                                                                            <label for="description">Sinopse</label>
                                                                        </div>
                                                                        <div class="form-floating mb-3">
                                                                            <input type="number" class="form-control" required step="5"
                                                                        ...
                                                                        <div class="mb-3">
                                                                            ...
                                                                            @foreach(['domingo', 'segunda', 'terça', 'quarta', 'quinta', 'sexta', 'sábado'] as $day)
                                                                            <div class="form-check ms-2">
                                                                                <input class="form-check-input checkbox-day-editar" type="checkbox" value="{{ $day }}" id="check-editar-{{ $day }}-{{ $card->id }}" name="days_editar[]"
                                                                                    data-card-id="{{ $card->id }}" {{ in_array($day, explode(',', $card->days)) ? 'checked' : '' }}>
                                                                                <label class="form-check-label" for="check-editar-{{ $day }}-{{ $card->id }}">{{ ucfirst($day) }}</label>
                                                                            </div>

                                                                            <!-- Horários -->
                                                                            <div id="horarios-editar-{{ $day }}-{{ $card->id }}" class="mt-2 ms-4 horarios-container-editar {{ in_array($day, explode(',', $card->days)) ? '' : 'd-none' }}">
                                                                                <div class="horario-wrapper-editar mb-3">
                                                                                    @if(!empty($card->horarios))
                                                                                    @foreach($card->horarios as $horario)
                                                                                    @if($horario->dia == $day)
                                                                                    <div class="d-flex align-items-center mb-2">
                                                                                        <input type="time" class="form-control me-2" name="horarios_editar[{{ $day }}][]" value="{{ \Carbon\Carbon::createFromFormat('H:i:s', $horario->horario)->format('H:i') }}">
                                                                                        <button type="button" class="btn btn-danger btn-remover-horario-editar" data-card-id="{{ $card->id }}">Remover</button>
                                                                                    </div>
                                                                                    @endif
                                                                                    @endforeach
                                                                                    @else
                                                                                    <div class="d-flex align-items-center mb-2">
                                                                                        <input type="time" class="form-control me-2" name="horarios_editar[{{ $day }}][]">
                                                                                        <button type="button" class="btn btn-danger btn-remover-horario-editar" data-card-id="{{ $card->id }}">Remover</button>
                                                                                    </div>
                                                                                    @endif
                                                                                </div>
                                                                                <button type="button" class="btn btn-success btn-adicionar-horario-editar d-flex align-items-center mb-3" data-dia="{{ $day }}" data-card-id="{{ $card->id }}">
                                                                                    <span class="ic--baseline-plus"></span>
                                                                                    <span class="roboto-regular">Adicionar horário</span>
                                                                                </button>
                                                                            </div>
                                                            ...
                                                            <div class="accordion-item">
                                                                ...
                                                                <h2 class="accordion-header">
                                                                    <button class="accordion-button collapsed" type="button"
                                                                        data-bs-toggle="collapse" data-bs-target="#collapseTwo{{ $card->id }}"
                                                                        aria-expanded="false" aria-controls="collapseTwo{{ $card->id }}">
                                                                        Imagens da Peça
                                                                    </button>
                                                                </h2>

                                                                <div id="collapseTwo{{ $card->id }}" class="accordion-collapse collapse"
                                                                    data-bs-parent="#accordionEdit">
                                                                    <div class="accordion-body">
                                                            ...
                                                            <div class="accordion-item">
                                                                <h2 class="accordion-header">
                                                                    <button class="accordion-button collapsed" type="button"
                                                                        data-bs-toggle="collapse" data-bs-target="#collapseThree{{ $card->id }}"
                                                                        aria-expanded="false" aria-controls="collapseThree{{ $card->id }}">
                                                                        Ficha Técnica
                                                                    </button>
                                                                </h2>
                                                                <div id="collapseThree{{ $card->id }}" class="accordion-collapse collapse"
                                                                    data-bs-parent="#accordionEdit">
                                                                    <div class="accordion-body">
                                                                        <div class="form-floating mb-3">
                                                                            <input type="text" class="form-control" required
                                                            ...
                                                            <div class="accordion-item">
                                                                <h2 class="accordion-header">
                                                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseFour{{ $card->id }}" aria-expanded="false" aria-controls="collapseFour{{ $card->id }}">
                                                                        Opcionais (Ficha Técnica)
                                                                    </button>
                                                                </h2>
                                                                <div id="collapseFour{{ $card->id }}" class="accordion-collapse collapse" data-bs-parent="#accordionEdit">
                                                                    <div class="accordion-body">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.checkbox-day').forEach(function(checkbox) {
            checkbox.addEventListener('change', function() {
                ...
                if (this.checked) {
                    ...
                    horariosDiv.querySelector('.horario-wrapper').innerHTML = ''; // .innerHTML = ''; // Remove os horários quando desmarcar o checkbox
                }
            });
        });
    ...
    document.querySelectorAll('.checkbox-day-editar').forEach(function(checkbox) {
        checkbox.addEventListener('change', function() {
            ...
            let horariosDiv = document.getElementById('horarios-editar-' + dia + '-' + cardId);
    ...
    document.querySelectorAll('.btn-adicionar-horario-editar').forEach(function(button) {
        button.addEventListener('click', function() {
            let dia = this.getAttribute('data-dia');
            let cardId = this.getAttribute('data-card-id');
            let horariosWrapper = document.querySelector('#horarios-editar-' + dia + '-' + cardId + ' .horario-wrapper-editar');
            ...
            if (horariosWrapper) {
                ...
                botaoRemover.textContent = 'Remover';
                botaoRemover.setAttribute('data-card-id', cardId);
